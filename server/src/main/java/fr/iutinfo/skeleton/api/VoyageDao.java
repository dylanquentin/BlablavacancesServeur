...
public interface VoyageDao {
	...
	@SqlUpdate("create table voyages (id integer primary key autoincrement, iduser varchar(20) ,name varchar(1024), ville varchar(100), description text, depart varchar(100), retour varvhar(100), capacite integer, idParticipe text)")
	void createVoyageTable();

	@SqlUpdate("insert into voyages (iduser,name,ville,description, depart, retour, capacite, idParticipe) values (:idUser, :name, :ville, :description, :depart, :retour, :capacite, :idParticipe)")
	@GetGeneratedKeys
	...
	@SqlUpdate("update voyages set name = :name, ville = :ville, description = :description, depart = :depart, retour = :retour, capacite = :capacite, idParticipe = :idParticipe where id = :id")
